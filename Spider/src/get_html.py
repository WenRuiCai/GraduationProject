...
import requests
import time
import Spider.src.config

cursor = 0

get_proxy_url = "http://piping.mogumiao.com/proxy/api/%s?appKey=%s&count=5&expiryDate=0&format=2&newLine=1"%(Spider.src.config.api_tyle,Spider.src.config.appkey)
proxy_id_list = []
proxy_id = ""
proxy_page = 0
...
def get_html(url, count):
    if count > Spider.src.config.html_retries:
        print("重试超过%d次：建议停机检查：" % Spider.src.config.html_retries, url)
        return ""
        # exit(1)
    # time.sleep(1)
    ...
    try:
        if len(Spider.src.config.appkey) is 0:
            response = requests.get(url, headers=headers)
        else:
            response = requests.get(url, headers=headers, proxies=proxy, timeout=Spider.src.config.request_timeout)
